...
class TestFMCDevices(common.TestCommon):
    ...
    def test_get_fmc_device_records_list(self):
        ...
        self.assertIsNotNone(self.csfw_client.get_fmc_device_records_list(self.domain_uuid))

    def test_get_fmc_device_record(self):
        """Given a device uudio, test getting the details of that device"""
        devices = self.csfw_client.get_fmc_device_records_list(self.domain_uuid)
        self.assertIsNotNone(self.csfw_client.get_fmc_device_record(self.domain_uuid, devices[0].id))
    ...
    def test_update_device_records(self):
        ...
        devices = self.csfw_client.get_fmc_device_records_list(self.domain_uuid)
        device = self.csfw_client.get_fmc_device_record(self.domain_uuid, devices[0].id)

        # Modify the device name
        original_name = device.name
        device.name = f"updated-{device.name}"
        updated_device = self.csfw_client.update_fmc_device_record(self.domain_uuid, device)
        self.assertEqual(device.name, updated_device.name)

        # Revert to the original device name
        updated_device.name = original_name
        reverted_device = self.csfw_client.update_fmc_device_record(self.domain_uuid, updated_device)
    ...
    def test_create_device_record(self):
        ...
        acp = self.csfw_client.get_access_policy_list(self.domain_uuid, name="Default Policy", expanded=False)[0]
        common.TEST_DEVICE.accessPolicy = acp
        self.assertIsNotNone(self.csfw_client.create_fmc_device_record(self.domain_uuid, common.TEST_DEVICE))
