...
import logging
import common
from fmcclient.models import NetworkObjectModel, HostObjectModel
...
class TestFMCNetworkObjects(common.TestCommon):
    ...
    def test_get_network_object_list(self) -> None:
        network_objs = self.fmc_client.get_network_objects_list(self.domain_uuid)
        if network_objs:
    ...
    def test_get_network_objects_list_filter(self) -> None:
        network_objs = self.fmc_client.get_network_objects_list(
    ...
    def test_get_network_object(self) -> None:
        ...
        self.assertIsInstance(self.fmc_client.get_network_object(self.domain_uuid, objects[0].id), NetworkObjectModel)
    ...
    def test_create_bulk_network_objects(self) -> None:
        net_objs = self.fmc_client.create_bulk_network_objects(self.domain_uuid, [common.NET_OBJ_1, common.NET_OBJ_2])
    ...
    def test_get_host_object_list(self) -> None:
        host_objs = self.fmc_client.get_host_objects_list(self.domain_uuid)
        if host_objs:
            for host_obj in host_objs:
                self.assertIsInstance(host_obj, HostObjectModel)

    def test_get_host_objects_list_filter(self) -> None:
        self.fmc_client.create_bulk_host_objects(self.domain_uuid, [common.HOST_OBJ_1, common.HOST_OBJ_2])
        host_objs = self.fmc_client.get_host_objects_list(self.domain_uuid, filter="nameOrValue:unittest-host-")
    ...
    def test_get_host_object(self) -> None:
        objects = self.fmc_client.get_host_objects_list(self.domain_uuid)
        self.assertIsInstance(self.fmc_client.get_host_object(self.domain_uuid, objects[0].id), HostObjectModel)

    def test_create_host_object(self) -> None:
        net_obj = self.fmc_client.create_host_object(self.domain_uuid, common.HOST_OBJ_1)
        self.assertIsInstance(net_obj, HostObjectModel)

    def test_create_bulk_host_objects(self) -> None:
        host_objs = self.fmc_client.create_bulk_host_objects(self.domain_uuid, [common.HOST_OBJ_1, common.HOST_OBJ_2])
    ...
    def test_delete_host_object(self) -> None:
        ...
        self.assertIsInstance(self.fmc_client.delete_host_object(self.domain_uuid, host_obj.id), HostObjectModel)

    def test_update_host_object(self) -> None:
        host_obj = self.fmc_client.create_host_object(self.domain_uuid, common.HOST_OBJ_1)
        host_obj.name = f"{host_obj.name}-updated"
        new_obj = self.fmc_client.update_host_object(self.domain_uuid, host_obj)
