...
class TestFMCAccessPolicies(common.TestCommon):
    ...
    def setUp(self):
        ...
        self.common_setup()
        self.ftd_ap = self.csfw_client.create_access_policy(self.domain_uuid, common.TEST_ACCESS_POLICY)

    def tearDown(self):
        """Remove the common objects created on the test FMC when tests are finished"""
        if self.ftd_ap:
            self.csfw_client.delete_access_policy(self.domain_uuid, self.ftd_ap.id)

    def test_get_ftd_access_policy_list(self):
        """Test the "Get list" of access p[oloicies."""
        self.assertIsNotNone(self.csfw_client.get_access_policy_list(self.domain_uuid, expanded=True))

    def test_get_ftd_access_policy_by_name_list(self):
        """Test the "get policy by name" feature of the get_access_policy_list method"""
        ap_list = self.csfw_client.get_access_policy_list(
    ...
    def test_get_ftd_access_policy(self):
        ...
        policy_list = self.csfw_client.get_access_policy_list(self.domain_uuid)
    ...
    def test_update_access_policy(self):
        ...
        """
        test_ftd_ap = self.csfw_client.get_access_policy(self.domain_uuid, self.ftd_ap.id)
        test_ftd_ap.name = f"updated-{self.ftd_ap.name}"
        updated_ftd_ap = self.csfw_client.update_access_policy(self.domain_uuid, test_ftd_ap)
    ...
    def test_get_ftd_access_rules_list(self):
        ...
        """
        test_ftd_ap = self.csfw_client.get_access_policy_list(
            self.domain_uuid, name=common.DEFAULT_ACCESS_CONTROL_POLICY
        )[0]
        self.assertIsInstance(self.csfw_client.get_access_rule_list(self.domain_uuid, test_ftd_ap.id), list)
    ...
    def test_get_ftd_access_rule(self):
        ...
        self.assertIsInstance(
            self.csfw_client.get_access_rule(self.domain_uuid, self.ftd_ap.id, access_rule_obj.id), FTDAccessRuleModel
        )
    ...
    def test_create_ftd_access_rule(self):
        ...
        self.assertIsInstance(
            self.csfw_client.create_access_rule(self.domain_uuid, self.ftd_ap.id, new_rule), FTDAccessRuleModel
        )
    ...
    def test_delete_ftd_access_rule(self):
        ...
        self.assertIsInstance(
            self.csfw_client.delete_access_rule(self.domain_uuid, self.ftd_ap.id, access_rule_obj.id),
            FTDAccessRuleModel,
        )
