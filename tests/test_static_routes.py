...
import common
import logging
from fmcclient.base import DuplicateObject
from fmcclient.models import HostObjectModel, IPv4StaticRouteModel, NetworkObjectModel
...
class TestFMCStaticRoutes(common.TestCommon):
                ...
                if route.selectedNetworks:
                    for network in route.selectedNetworks:
                        if network["name"] == common.NET_OBJ_1.name or network["name"] == common.NET_OBJ_2.name:
                            self.fmc_client.delete_ipv4_static_route(self.domain_uuid, self.device.id, route.id)
                            break
    ...
    def create_test_host_objects(self) -> list[HostObjectModel]:
        try:
            return self.fmc_client.create_bulk_host_objects(self.domain_uuid, [common.HOST_OBJ_1, common.HOST_OBJ_2])
        except DuplicateObject:
            log.warning("Host object(s) already exists. Continuing....")
            return self.fmc_client.get_host_objects_list(self.domain_uuid, filter="nameOrValue:unittest-host-")

    def create_test_network_objects(self) -> NetworkObjectModel:
        try:
            return self.fmc_client.create_bulk_network_objects(self.domain_uuid, [common.NET_OBJ_1, common.NET_OBJ_2])
        except DuplicateObject:
            log.warning("Network objects already exist. Continuing....")
            return self.fmc_client.get_network_objects_list(self.domain_uuid, filter="nameOrValue:unittest-network-")
    ...
    def create_test_static_route(self) -> IPv4StaticRouteModel:
        ...
        return self.fmc_client.create_ipv4_static_route(
            self.domain_uuid,
            self.device.id,
    ...
    def test_get_ipv4_static_route(self) -> None:
        ...
        self.assertIsInstance(
            self.fmc_client.get_ipv4_static_route(self.domain_uuid, self.device.id, static_route.id),
            IPv4StaticRouteModel,
        )
        # self.fmc_client.delete_ipv4_static_route(self.domain_uuid, self.device.id, static_route.id)
    ...
    def test_delete_ipv4_static_route(self) -> None:
        ...
        deleted_route = self.fmc_client.delete_ipv4_static_route(self.domain_uuid, self.device.id, test_static_route.id)
        self.assertIsInstance(deleted_route, IPv4StaticRouteModel)

    def test_update_static_route(self) -> None:
        test_static_route = self.create_test_static_route()
        net_obj_2 = self.fmc_client.get_network_objects_list(
        ...
        updated_static_route = self.fmc_client.update_ipv4_static_route(
