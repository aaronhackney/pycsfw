...
class TestFMCInterfaces(common.TestCommon):
    ...
    def delete_test_sub_iface(self):
        sub_int_list = self.fmc_client.get_ftd_subiface_list(self.domain_uuid, self.device.id)
        if sub_int_list:
            ...
            if dup_int:
                ...
                self.fmc_client.delete_ftd_subiface(self.domain_uuid, self.device.id, dup_int[0].id)
    ...
    def reset_physical_interface_config(self):
        ...
        test_iface = self.fmc_client.get_ftd_physical_iface_by_name(
        ...
        self.fmc_client.update_ftd_physical_iface(self.domain_uuid, self.device.id, reset_iface)
    ...
    def test_get_device_phys_interface_by_name(self):
        """Test getting an interface's details, given an interface's uuid"""
        iface = self.fmc_client.get_ftd_physical_iface_by_name(
            self.domain_uuid, self.device.id, common.TEST_PHYSICAL_IFACE_CONFIG.name
        )
    ...
    def test_get_sub_iface_list(self):
        ...
        self.assertIsInstance(self.fmc_client.get_ftd_subiface_list(self.domain_uuid, self.device.id), list)
    ...
    def test_delete_subinterface(self):
        ...
        deleted_iface = self.fmc_client.delete_ftd_subiface(self.domain_uuid, self.device.id, sub_iface.id)
