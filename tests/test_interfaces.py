...
class TestFMCInterfaces(common.TestCommon):
    ...
    def delete_test_sub_iface(self):
        sub_int_list = self.csfw_client.get_ftd_subiface_list(self.domain_uuid, self.device.id)
        if sub_int_list:
            dup_int = [sub_iface for sub_iface in sub_int_list if sub_iface.ifname == common.SUB_IFACE_CONFIG.ifname]
            if dup_int:
                log.warning("Deleting test sub-interface")
                self.csfw_client.delete_ftd_subiface(self.domain_uuid, self.device.id, dup_int[0].id)

    def create_sub_interface(self):
        try:
            sub_iface = self.csfw_client.create_ftd_subiface(self.domain_uuid, self.device.id, common.SUB_IFACE_CONFIG)
        except DuplicateObject as ex:
            log.warning("Test Sub-Interface already existed...")
            self.delete_test_sub_iface()
            sub_iface = self.csfw_client.create_ftd_subiface(self.domain_uuid, self.device.id, common.SUB_IFACE_CONFIG)
        return sub_iface
    ...
    def reset_physical_interface_config(self):
        ...
        test_iface = self.csfw_client.get_ftd_physical_iface_by_name(
            self.domain_uuid, self.device.id, common.TEST_PHYSICAL_IFACE_CONFIG.name
        )
        reset_iface = common.TEST_PHYSICAL_IFACE_CONFIG
        reset_iface.id = test_iface.id
        self.csfw_client.update_ftd_physical_iface(self.domain_uuid, self.device.id, reset_iface)

    def test_get_physical_interface_list(self):
        """Test getting list of physical interfaces"""
        self.assertIsInstance(self.csfw_client.get_ftd_physical_iface_list(self.domain_uuid, self.device.id), list)

    def test_get_device_phys_interface(self):
        """Test getting an interface's details, given an interface's uuid"""
        interfaces = self.csfw_client.get_ftd_physical_iface_list(self.domain_uuid, self.device.id)
        self.assertIsInstance(
            self.csfw_client.get_ftd_physical_iface(self.domain_uuid, self.device.id, interfaces[0].id),
    ...
    def test_update_phys_interface(self):
        ...
        updated_iface = self.csfw_client.update_ftd_physical_iface(self.domain_uuid, self.device.id, iface)
    ...
    def test_get_vlan_interfaces_list(self):
        ...
        self.assertIsNotNone(self.csfw_client.get_ftd_vlan_iface_list(self.domain_uuid, self.device.id))

    def test_get_sub_iface_list(self):
        """Test getting a list of all sub interterfaces"""
        self.create_sub_interface()
        self.assertIsInstance(self.csfw_client.get_ftd_subiface_list(self.domain_uuid, self.device.id), list)
    ...
    def test_create_subinterface(self):
        ...
        self.delete_test_sub_iface()
        sub_iface = self.csfw_client.create_ftd_subiface(self.domain_uuid, self.device.id, common.SUB_IFACE_CONFIG)
        self.assertIsInstance(sub_iface, FTDSubInterfaceModel)
    ...
    def test_update_subinterface(self):
        ...
        updated_subif = self.csfw_client.update_ftd_subiface(self.domain_uuid, self.device.id, new_sub_int)
    ...
    def test_delete_subinterface(self):
        ...
        deleted_iface = self.csfw_client.delete_ftd_subiface(self.domain_uuid, self.device.id, sub_iface.id)
